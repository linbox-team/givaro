...
struct Frac {
  ...
  T1& nume() { return _num; }
  const T1& nume() const { return _num; }
  T2& deno() { return _den; }
  const T2& deno() const { return _den; }
};
...
//         Ring_t::write(std::cerr << "g: ", g) << std::endl;
    ...
    int isZero  ( const Rep& P ) const { return Ring_t::isZero(P._num); }
    int isOne   ( const Rep& P ) const { return Ring_t::areEqual(P._num, P._den); }
    ...
    Rep& negin ( Rep& res ) const { Ring_t::negin(res._num); return res; }
